...
class Config(pydantic.BaseModel):
    ...
    classifier: str | None = pydantic.Field("SmolLM2-135M", description="Name of model used for training and inference.")
    ...
    num_labels: int | None = pydantic.Field(None, description="Number of unique labels, if 'None' will be inferred from train set.")
    ...
    device: str = pydantic.Field(
        ...
        description="Whether to use GPU or CPU."
    )
    seed: int = pydantic.Field(10, description="Seed to use for reproducibility.")
    custom_model_config_path: pathlib.Path | None = pydantic.Field(None, description="Path to load model with custom config, only used if pretrained_model=False")
    custom_tokenizer_config_path: pathlib.Path | None = pydantic.Field(None, description="Path to load tokenizer with custom config, only used if pretrained_tokenizer=False")
    ...
    early_stopping_patience: int = pydantic.Field(3, description="Number of evaluations with no improvement after which training will be stopped")
    ...
    gradient_accumulation_steps: int | None = pydantic.Field(1, description="Number of steps to accumulate gradients before performing a backward/update pass")
